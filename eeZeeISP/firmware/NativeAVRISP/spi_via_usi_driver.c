...
void spiX_initmaster( char spi_mode )
{
	...
	USI_DIR_REG |= (1<<USI_DATAOUT_PIN)|
				   (1<<USI_CLOCK_PIN)|
				   (1<<USI_SS_PIN);				// Outputs.
	USI_DIR_REG &= ~(1<<USI_DATAIN_PIN);        // Inputs.
	USI_OUT_REG |= (1<<USI_DATAIN_PIN)|			// Pull-up.
				   (1<<USI_SS_PIN);         	// Slave select active low
	
	// Configure USI to 3-wire master mode with overflow interrupt.
	USICR = (1<<USIOIE)|					// overflow interrupt enable
			(1<<USIWM0)|					// 3-wire mode
			(1<<USICS1)|(spi_mode<<USICS0)|	// external clock mode
	        (1<<USICLK);					// Software clock strobe (USITC)
...
void spiX_initslave( char spi_mode )
{
	...
	USICR = (1<<USIOIE)|					// overflow interrupt enable
			(1<<USIWM0)|					// 3-wire mode
	        (1<<USICS1)|(spi_mode<<USICS0);	// clock mode
